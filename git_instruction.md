...
***git remote add origin https://github.com/Ilshat1991/1234.git*** - эта команда связывает  локальный и удаленный репозитории. Ссылка указывает на удалённый репозиторий.

***git branch -M main***  - эта команда переименовывает текущую ветку в ***main***. Желательно переименовать ветку ***master*** в ветку ***main***.

***git push -u origin main*** - после этой команды git запомнит, что отправлять надо ветку под именем ***main***. 
...
***git clone*** - полностью копирует удалённый репозиторий. Для этого после команды ***git clone*** необхдимо ввести ссылку на удалённый репозиторий, который хотите скопировать. Например ***git clone https://github.com/Ilshat1991/gitHomeWork.git***. 

В GitHub, что бы найти ссылку нужно зайти в интересующий вас репозиторий и нажать кнопку ***Code***.

***git pull*** - эта команда скачивает все изменения с удалённого репозитория на локальный. После этой команды нужно ввести ссылку удалённого репозитория.

***git push*** - эта команда отправляет все актуальные изменения из локального репозитория в удалённый.

***git push origin branch:server_branch*** - эта команда позволяет отправить данные из локальной ветки в удалённую ветку с другим именем. Вместо ***branch*** нужно ввести имя локальной ветки, вместо ***server_branch*** имя удалённой ветки.

***git push origin --all*** - эта команда позволяет отправить все ветки из локального репозитория на  удалённый.

***git push origin HEAD*** - эта команда позволяет отправить текущую ветку из локального репозитория, не водя название ветки. Имя удалённой ветки должно совпадать с именем локальной ветки.
...
## Fork
Форк  — собственное ответвление  какого-то проекта. Это означает, что GitHub создаст вашу собственную копию проекта, данная копия будет находиться в вашем удалённом репозитории и вы сможете вносить изменения в неё. Для того, что бы создаль копию, найдите интересующий вас репозиторий и нажмите кнопку ***fork***.