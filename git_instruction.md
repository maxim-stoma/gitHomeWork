...
В GITHAB есть кнопка Code, нажимая на которую мы увидим ссылку, пример на фото :

![clone](clone.png)

Копируем ссылку, идем в VSC в терминале прописывем _**git clone**_ и скачивается репозиторий на локальный компьютер.
...
Появится строка о том, что были произведены изменения, а так предложение Compare & Pull request, нажав на которое будет переход на страничку с созданием Pull request. 

Заполняем необходимые поля, нажимаем кнопку и ждем сообщения от владельца репозитория.
