...
4. После того, как все желаемые изменения внесены и выполнены, уже готовый репозиторий нужно "запушить" на GitHub. Это действие выполняется с помощью команды *git push*. 

5. И, наконец, пользватель должен предложить свои измения автору изначального проекта. Как правилло, после того, как готовый репозиторий был "запушен", на странице в GitHub автоматически появляется сообщение о предложении отправить запрос автору изначального репозитория *compare and request*. После начажатия на эту кнопку, появится окно для комментария пользвователя, который прикрепится к его риквесту. 
