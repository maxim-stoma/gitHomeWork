...
### Просмотр удаленных репозиториев. Команда git clone.

Для того, чтобы получить доступ к удаленному репозиторию и иметь возможность внести в него какие-либо изменения, пользователю нужно совершить несколько действий:

1. На странице интересующего его репозитория на GitHub создать копию оригинального репозитория, в которой пользователь в дальнейшем будет работать, прежде чем предложить изменения создателю оригинального репозитория. Для этого необходимо нажать на кнопку **fork** на странице репозитория (Рис 1).

![Иллюстрация кнопки fork](request1.bmp)

2. После этого необходимо клонировать копию репозитория на локальный компьютер. Это делается с помощью команды *git clone <ссылка на копию репозитория на GitHub>* (Рис2).

![Иллюстрация git clone](request2.bmp)

3. После добавления, пользователю следует создать новую ветку, а не вносить изменения в ветку *master/main*. 

4. После того, как все желаемые изменения внесены и выполнены, уже готовый репозиторий нужно "запушить" на GitHub. Это действие выполняется с помощью команды *git push* (Рис3).

![Иллюстрация git push](request3.bmp)

5. И, наконец, пользватель должен предложить свои измения автору изначального проекта. Как правилло, после того, как готовый репозиторий был "запушен", на странице в GitHub автоматически появляется сообщение о предложении отправить запрос автору изначального репозитория *compare and pull request*. После начажатия на эту кнопку, появится окно для комментария пользвователя, который прикрепится к его риквесту (Рис 4,5).

![Иллюстрация pull request](request4.bmp)

![Иллюстрация conversation](request5.bmp)

После всех перечисленных шагов, автор изначального репозитория должен либо принять, либо отклонить изменения. И в том, и в другом случае автор имеет возможность также написать комментарий к риквесту. Таким образом, между пользователями имеется возможность поддерживать связь и обсуждать проект.

![иллюстрация comments](request6.bmp)

## Заключение

Мы рассмотрели основные команды, необходимые для работы с локальным и удаленным репозиториями. 