...
Чтобы удалить ветку надо использовать команду *git branch -d <имя_ветки>* (где <имя_ветки> заменяем на имя нашей удаляемой ветки):
...
Репозиторий может быть не только локальным, но и __*удаленным*__. Удаленный репозиторий - это такой репозиторий, который хранится не только у нас на компьютере, но и на сервере в сети Интернет. К такому репозиторию обычно имеют доступ несколько пользователей и все они совместно работают с ним.

Чтобы клонировать удаленный репозиторий надо использовать команду *git clone <адрес_репозитория>* (где <адрес_репозитория> заменяем на имя нашего удаленного репозитория):

```
PS C:\Users\einoz\Documents\Обучение\Введение в контроль версий\Семинар 3\gitHomeWork> git clone <адрес_репозитория>
```

### Добавление удаленного репозитория для локального репозитория
***

Если у нас уже есть локальный репозиторий (на нашем компьютере) и мы хотим его отправить на удаленный репозиторий (сервере в сети Интернет), то нам нужно использовать команду *git remote add <имя_удаленного_репозитория> <адрес_репозитория>* (где <имя_удаленного_репозитория> заменяем на имя удаленного репозитория, а <адрес_репозитория> заменяем на имя нашей удаленного репозитория):

```
PS C:\Users\einoz\Documents\Обучение\Введение в контроль версий\Семинар 3\gitHomeWork> git remote add <имя_удаленного_репозитория> <адрес_репозитория>
```
...
После того, как мы внесли изменения в локальном репозитории и зафиксировали их, нам нужно отправить эти изменения в удаленный репозиторий.

Чтобы отправить изменения в удаленный репозиторий надо использовать команду *git push -u <имя_удаленного_репозитория> <имя_ветки>* (где <имя_удаленного_репозитория> заменяем на имя нашего удаленного репозитория, а <имя_ветки> на имя ветки):

```
PS C:\Users\einoz\Documents\Обучение\Введение в контроль версий\Семинар 3\gitHomeWork> git push -u <имя_удаленного_репозитория> <имя_ветки>
```

### Получение изменений удаленного репозитория
***

Поскольку с нашим удаленным репозиторием можем работать не только мы, но и другие пользователи нам нужно иметь возможность получать изменения удаленного репозитория.

Чтобы получить изменения в удаленный репозиторий надо использовать команду *git pull*:

```
PS C:\Users\einoz\Documents\Обучение\Введение в контроль версий\Семинар 3\gitHomeWork> git pull
```

Эта команда является составной - она не только получает изменения удаленного репозитория, но и пытается выполнить слияние с нашей локальной веткой (merge).

### Предложенные изменения
***

Не всегда у нас есть возможность и права доступа на то, чтобы отправлять свои изменения в удаленный реозиторий. Обычно каждый разработчик работает со своей веткой (и может быть даже не одной) и отправляет в них свои изменения. Однако когда эти изменения надо будет отправить в общие ветки для сборки всего приложения или отдельной его части, то у разработчика может не быть такой возможности (прав доступа на это).

Вместо этого разработчик может предложить свои изменения (в другую ветку текущего репозитория или в другой репозиторий, из которого разработчик получил свой репозиторий (выполнил fork)).

Это делается с помощью команды pull request. В результате выполнения этой команды разработчик, который ответственнен за целевой репозиторий (или ветку целевого репозитория), получает запрос на слияение и анализирует его. Если с изменениями все в порядке - они нужны, выполнены правильно и без ошибок - то разработчик может выполнить слияение и эти изменения попадут в основную ветку репозитория (или в целево репозиторий). Если эти изменения не нужны или выполнены не совсем правильно, то pull request можно отклонить и тогда эти изменения не попадут в целевую ветку или репозиторий.

Разработчик, который собирается выполнить изменения и предложить их в последующем для слияния должен делать это в отдельной ветке репозитория после выполнения fork.
