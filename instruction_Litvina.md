...
Теперь нужный репозиторий скопирован в локлаьный на нашем устройстве. Теперь осталось создать новую ветку (это нормы приличия при работе с чужим файлом) и произвести неоходимые изменения на этой ветке.

Когда мы произвесли все необходимые изменения, осталось только передать наш локлаьный репозиторий обратно на GitHub и далее направить нашу версию автору, который может принять или не принять нашу версию. Это делает через езлёную кнопку на картинке ниже.
