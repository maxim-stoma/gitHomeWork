...
## Пример команд для удалённой синхронизации с GitHub
  ...
  5. Отправить _git push_ локальный репозиторий в удалённый (на GitHub).
  6. Выкачать _git pull_ актуальное состояние из удалённого репозитория. 

  ### Чтобы предложить изменние в проект пренадлежащий другому пользователю GitHub (pull request)
...
**Все файлы, которые должны быть проигнорированы, сохраняются в файле _.gitignore_**.
...
Например: запись которая игнорирует все файлы с расширением png в нутри репозитория:
